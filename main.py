...
def enrich_issue(issue, project_name, headers):
    ...
    issue['notes'] = fetch_notes(GITLAB_API_URL, issue['project_id'], issue['iid'], headers)
    issue['ai_summary'] = summarize_issue(issue)
    return issue

def main(project_id):
    headers = {"PRIVATE-TOKEN": GITLAB_TOKEN}  # build headers once here

    now = datetime.now(timezone.utc)
    since = (now - timedelta(days=2)).isoformat()

    project = fetch_project_info(GITLAB_API_URL, project_id, headers)
    if not project:
        print("Invalid project ID")
        return
    project_name = project.get('name', 'Unknown Project')

    members = fetch_project_members(GITLAB_API_URL, project_id, headers)
    final = {}
    ...
    for member in members:
        ...
        seen = set()
        issues = fetch_issues(GITLAB_API_URL, project_id, uid, since, headers)
        for issue in issues:
            seen.add(issue['id'])

        for issue in fetch_issues_by_username(GITLAB_API_URL, project_id, uname, since, headers):
            if issue['id'] not in seen:
                issues.append(issue)
                seen.add(issue['id'])

        for issue in fetch_authored_issues(GITLAB_API_URL, project_id, uid, since, headers):
            if issue['id'] not in seen:
                issues.append(issue)
                seen.add(issue['id'])

        for issue in fetch_all_project_issues(GITLAB_API_URL, project_id, since, headers):
            if issue['id'] in seen:
                continue
